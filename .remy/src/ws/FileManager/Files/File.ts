...
export class File extends BaseFile {
  ...
  async open(editorType?: EditorType) {
    ...
    const stat = fs.statSync(`${this.projectPath}/${this.realPath}`);
    if (stat.size > 5e6) {
    ...
    fs.readFile(`${this.projectPath}/${this.realPath}`, { encoding: 'utf8' }, (err, data) => {
      if (!err) {
        this.body = data;
        this.broadcastContents();
      }
  ...
  async save(): Promise<any> {
    ...
    return new Promise<any>((resolve, reject) => {
      fs.writeFile(`${this.projectPath}/${this.realPath}`, this.body, (err) => {
        resolve();
      });
    });
  }
